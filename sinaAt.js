(function (window) {
    ...
    function at(textarea, options) {
        ...
        document.body.appendChild(atList)
        ...
        if (textarea.addEventListener) {
            textarea.addEventListener("keydown", this.keydown.bind(this), false)
            ...
            textarea.attachEvent("onkeydown", this.keydown.bind(this))
    ...
    at.prototype.objChange = function (event) {
        ...
        loadPositions = function () {
            //取值
            objString = textarea.value,
            //记录光标当前位置
            cursorPosition = posCursor(textarea),
            //光标之前的字符串
        ...
        if (atList.style.display == "block") {
            var listClick = toArray(atList.getElementsByTagName("li"))
        ...
        if (beforeCursorString.indexOf(that.at) != -1 && indexString.indexOf(that.seperator) == -1 && indexString.indexOf('\n') == -1) {
            ...
            fetchList(query, function (list) {
                ...
                if (list.length === 0) {
                    ...
                    if (query === '') {
                        // 开始查询，提示他查询
                        atList.innerHTML = '<li class="list-title">' + that.title + '</li>'
                    } else {
                        // 查询无结果，提示无结果
                        atList.innerHTML = '<li class="list-title">' + that.emptyTitle + '</li>'
                    }
                } else if (list.length > 0) {
                    // 渲染atList
                    atList.innerHTML = list.reduce(function (dom, item) {
                        return dom + '<li class="list-content">' + that.getText(item) + '</li>'
                    }, '<li class="list-title">' + that.title + '</li>')
                    // 绑定鼠标事件
                    var listClick = toArray(atList.getElementsByTagName("li"))
                ...
                hiddenObj.innerHTML = positionString.replace(/\n/g, "<br/>") + '<span class="at">' + that.at + '</span>'
                var at = that.parent.getElementsByTagName('span')[0]
    ...
    at.prototype.keydown = function (e) {
        if (this.atList.style.display == 'block') {
    ...
    at.prototype.handleString = function (index, selectedStr, atLocation, cursorPosition) {
        ...
        if (this.addAtData(data, atLocation)) {
            ...
            this.onAtChange(this.getAtData())
    ...
    at.prototype.addAtData = function (data, atLocation) {
        var index = this.atData.findIndex(function (data) {
    ...
    at.prototype.removeAtData = function (prevCursor, nowCurcor) {
        var removeAtData = this.atData.filter(function (data) {
            return data.location < prevCursor && data.end  >= nowCurcor
        })
        var startLocation = removeAtData[0] && removeAtData[0].location
        removeAtData.forEach(function (data) {
            var index = this.atData.findIndex(function (d) {
                return d === data
            })
            this.atData.splice(index, 1)
        }.bind(this))
    ...
    at.prototype.getAtData = function () {
        return this.atData.map(function (atData) {
            return atData.data
        })
    }
    ...
    ; (function polyfill() {
        ...
        Array.prototype.filter = Array.prototype.filter || function (condition) {
            var result = []
            for (var i = 0; i < this.length; i++) {
                if (condition(this[i])) {
                    result.push(this[i])
